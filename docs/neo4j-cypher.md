...
> You might need to download a JVM (Open JDK or Oracle JDK).
  ...
  ```
  - On windows, you can [download](http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) the Oracle JDK v8

### Cypher

[Cypher](https://neo4j.com/developer/cypher-query-language/) is the query language for neo4j, it is an SQL-inspired language for describing patterns in graphs visually using an ascii-art syntax.
...
There are multiple [options](https://neo4j.com/developer/language-guides/) to interact with Neo4j.

With python 3, you can use the python driver with [py2neo](http://py2neo.org/v3/) a client library and toolkit to communicate with neo4j.
...
Py2neo in the example works with [bottle](https://bottlepy.org/docs/dev/#) which is a micro web-framework to interact with the Neo4j database via its Rest API.
...
The default [neo4j browser](https://neo4j.com/developer/guide-neo4j-browser/) allows to display nicely the results of a Cypher query with the built-in [D3.js](https://d3js.org/) library.
...
- Library for neo4j and D3.js: [neo4jd3](https://github.com/eisman/neo4jd3)
- Some examples:
...
You can use the minified D3.js library with:
```html
